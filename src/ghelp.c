...
void onlinehelp(int standalone,int context,GTOBJECT *gt)
{
  ...
  char *songkeys[] = {
    ...
    "SHIFT+1-6 Swap orderlist with chn.",
    NULL
  };
  ...
  for (;;)
  {
    ...
    clearscreen(getColor(7, 0));
    if(!context) {
      printtext(0, left++, getColor(HELP_HEADER,0), "GENERAL KEYS");
      left = printrows(0,left, getColor(7,0), genkeys);
      left++;

      printtext(40,right++, getColor(HELP_HEADER,0), "PATTERN EDIT MODE");
      right = printrows(40,right, getColor(7,0), patternkeys);
      right++;
    
      printtext(0, left++, getColor(HELP_HEADER,0), "SONG EDIT MODE");
      left = printrows(0,left, getColor(7,0), songkeys);
      left++;
      ...
      printtext(40,right++, getColor(HELP_HEADER,0), "INSTRUMENT/TABLE EDIT MODE");
      right = printrows(40,right, getColor(7,0), instkeys);
      right++;
    
      left = (left<right ? right : left);
    
      printtext(0, left++, getColor(HELP_HEADER,0), "PATTERN COMMANDS");
      left = printrows(0,left, getColor(7,0), pattcmds);
      left++;

      printtext(0, left++, getColor(HELP_HEADER,0), "INSTRUMENT PARAMETERS");
      left = printrows(0,left, getColor(7,0), instparm);
      left++;

      printtext(0, left++, getColor(HELP_HEADER,0),"TABLES");
      left = printrows(0,left, getColor(7,0), tables);
      left++;
    } else {
      switch(editorInfo.editmode) {
      case EDIT_PATTERN:      
    printtext(0,left++, getColor(HELP_HEADER,0), "PATTERN EDIT MODE");
    left = printrows(0,left, getColor(7,0), patternkeys);
    left++;
    printtext(0, left++, getColor(HELP_HEADER,0), "PATTERN COMMANDS");
    left = printrows(0,left, getColor(7,0), pattcmds);
    left++;
    ...
    if(standalone) {
      printtext(55, 0, getColor(15, 1), "Arrows/PgUp/PgDn/Home/End scroll, ESC exits");
    } else {
      printtext(34, 0, getColor(15, 1), "Arrows/PgUp/PgDn/Home/End scroll, F12 toggles context, others exit");
    }
    printbg(0, 0, getColor(15, 1), MAX_COLUMNS);
